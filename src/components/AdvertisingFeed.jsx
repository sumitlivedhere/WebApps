...
function AdvertisingCardItem({ item, selectedCity, onSelect, getMessageTemplate }) {
  ...
  const handleStarClick = (e) => {
    ...
    hyperlocalStore.incrementInterest(
      ...
      item.agencyName || item.sellerName || 'Ad Agency / Space Owner'
    );
  };
    ...
    >
      ...
      <div className="relative h-48 w-full bg-slate-100 rounded-2xl overflow-hidden shadow-inner select-none">
        ...
        <span className="absolute bottom-2.5 left-2.5 text-xs font-black px-2.5 py-1 rounded-xl text-white bg-slate-950/85 backdrop-blur-md border border-white/10">
          {item.price || item.rates || 'Rates on Inquiry'}
        </span>
      ...
      <div className="pt-0.5 space-y-1.5">
        <div className="flex items-start justify-between">
          ...
          <span className="text-[10px] font-bold text-amber-800 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-md shrink-0 ml-2">
            {(item.subCategory || 'PROMO')
              .replace('private-space-', '')
              .replace('brands-seeking-', '')
              .toUpperCase()}
          </span>
        </div>
...
export default function AdvertisingFeed({
  ...
  const getMessageTemplate = (item) => {
    ...
    if (sub.includes('private-space')) {
      return `Namaste ${item.agencyName || item.sellerName || ''}, I saw your space listing "${item.name || item.title}" on TownHub. I want to inquire regarding renting your rooftop/wall for advertising in ${selectedCity}.`;
    }
    if (sub.includes('brands-seeking')) {
      return `Namaste, I saw your brand requirement "${item.name || item.title}" on TownHub. I have a high-traffic space available in ${selectedCity}. Can we discuss?`;
    }
    if (sub.includes('newspaper')) {
      return `Namaste ${item.agencyName || item.sellerName || ''}, I want to book a classified / display advertisement in ${selectedCity}.`;
    }
      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500">
            {filteredListings.length} verified spaces, printers & ad networks in {selectedCity}
          </p>
        </div>
        <button
          type="button"
