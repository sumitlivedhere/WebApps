...
export default function MarketFeed({
  ...
  const filteredProducts = useMemo(() => {
    ...
    return (products || []).filter((item) => {
      ...
      const itemSub = (item.subCategory || item.productCategory || item.sub_category || '').toLowerCase().trim();
  ...
  }, [products, selectedSubCategory, selectedCity, searchQuery]);
      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
        <div>
          <h2 className="text-sm font-black text-slate-900 capitalize">
            {selectedSubCategory !== 'all' ? selectedSubCategory.replace('-', ' ') : 'Market Products'}
          </h2>
          <p className="text-[10px] text-slate-500">Retail & Local Market in {selectedCity}</p>
        </div>
        <button
          type="button"
          onClick={onBack}
      ...
      {filteredProducts.length === 0 ? (
        <div className="bg-white/80 rounded-2xl p-8 text-center border border-slate-200">
          ...
          <p className="text-slate-600 font-bold text-xs mt-2">
            No products found in this category for {selectedCity}.
          </p>
        </div>
      ) : (
          ...
          >
            <div className="relative h-48 w-full bg-slate-100 rounded-2xl overflow-hidden shadow-inner">
              <img
                ...
                onError={(e) => {
                  e.target.src = 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=700';
                }}
              />
            ...
            <div className="pt-0.5">
              <div className="flex items-start justify-between">
                <h3 className="font-black text-slate-900 text-sm">{p.name || p.title}</h3>
              ...
              <div className="flex items-center justify-between text-[10px] text-slate-500 font-semibold mt-2 pt-2 border-t border-slate-100">
                ...
                <span className="text-emerald-700 font-bold">{p.distance || '0.1 km away'}</span>
              </div>
            </div>
            ...
            <ActionButtons
              ...
              message={`Namaste, I want to inquire about "${p.name || p.title}".`}
            />
          </article>
        ))
      )}
