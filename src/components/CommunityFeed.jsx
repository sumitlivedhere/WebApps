...
function CommunityCardItem({ item, selectedCity, onSelect, getMessageTemplate }) {
  ...
  const handleStarClick = (e) => {
    ...
    hyperlocalStore.incrementInterest(
      ...
      item.organizerName || item.sellerName || 'Volunteer Coordinator'
    );
  };
  ...
  return (
    <article
      ...
      className={`bg-white rounded-2xl overflow-hidden shadow-xs border transition p-3.5 space-y-3 relative cursor-pointer hover:shadow-md active:scale-99 ${
        item.isNew ? 'border-rose-400 ring-2 ring-rose-400/20' : 'border-slate-200'
      }`}
    >
      {/* Photo Banner */}
...
export default function CommunityFeed({
  ...
  const categoryConfig = getCategoryById('community') || { subCategories: [] };
  ...
  const getMessageTemplate = (item) => {
    return `Namaste, I am reaching out regarding your community initiative "${item.name || item.title}" on TownHub Seva in ${selectedCity}. How can I contribute / seek help?`;
  };
      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500">
            {filteredListings.length} verified non-profit initiatives & trusts in {selectedCity}
          </p>
        </div>
        <button
          type="button"
