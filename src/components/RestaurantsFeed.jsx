...
function RestaurantCardItem({ item, selectedCity, onSelect, getMessageTemplate }) {
  ...
  const handleStarClick = (e) => {
    ...
    hyperlocalStore.incrementInterest(
      ...
      item.sellerName || item.title || item.name || 'Restaurant'
    );
  };
    ...
    >
      ...
      <div className="relative h-48 w-full bg-slate-100 rounded-2xl overflow-hidden shadow-inner select-none">
        <img
          ...
          onError={(e) => {
            e.target.src = 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=700';
          }}
        />

        {/* Price / Price for Two Tag */}
        <span className="absolute bottom-2.5 left-2.5 text-xs font-black px-2.5 py-1 rounded-xl text-white bg-slate-950/85 backdrop-blur-md border border-white/10">
          {item.price || item.priceForTwo || item.rates || '₹ 400 for Two'}
        </span>
      ...
      <div className="pt-0.5 space-y-1.5">
        <div className="flex items-start justify-between">
          ...
          <span className="text-[10px] font-bold text-amber-800 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-md shrink-0 ml-2">
            {String(item.subCategory || 'FOOD').toUpperCase()}
          </span>
        </div>
...
export default function RestaurantsFeed({
  ...
  const categoryConfig = getCategoryById('restaurants') || { subCategories: [] };
  ...
  const getSubCategoryTitle = () => {
    if (targetSub === 'all') return 'All Restaurants & Food Spots';
  ...
  const getMessageTemplate = (item) => {
    return `Namaste ${item.sellerName || item.title || item.name || ''}, I want to book a table / inquire about menu offerings for "${item.title || item.name}" seen on TownHub in ${selectedCity}.`;
  };
      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500">
            {filteredRestaurants.length} live verified food spots & menus in {selectedCity}
          </p>
        </div>
        <button
          type="button"
      ...
      {filteredRestaurants.length === 0 ? (
        <div className="bg-white/80 rounded-2xl p-8 text-center border border-slate-200">
          ...
          <p className="text-slate-600 font-bold text-xs mt-2">
            No restaurants found under {targetSub !== 'all' ? targetSub : 'this category'} in {selectedCity}.
          </p>
        </div>
      ) : (
