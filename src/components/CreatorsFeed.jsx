...
function CreatorsCardItem({ item, selectedCity, onSelect, getMessageTemplate }) {
  ...
  const handleStarClick = (e) => {
    ...
    hyperlocalStore.incrementInterest(
      ...
      item.name || item.sellerName || 'Digital Creator / Freelancer'
    );
  };
  ...
  const gallery =
    item.images && item.images.length > 0
      ...
      : item.image
      ? [item.image]
      : ['https://images.unsplash.com/photo-1574717024653-61fd2cf4d44d?w=700'];
  ...
  return (
    <article
      ...
      className={`bg-white rounded-2xl overflow-hidden shadow-xs border transition p-3.5 space-y-3 relative cursor-pointer hover:shadow-md active:scale-99 ${
        item.isNew ? 'border-violet-400 ring-2 ring-violet-400/20' : 'border-slate-200'
      }`}
    >
      {/* Photo Banner */}
      ...
      <div className="pt-0.5 space-y-1.5">
        <div className="flex items-start justify-between">
          <div className="min-w-0">
            ...
            {item.experience && (
              <p className="text-[10px] text-violet-700 font-bold mt-0.5 truncate">
                ⭐ {item.experience}
              </p>
            )}
          </div>
          <span className="text-[10px] font-bold text-violet-800 bg-violet-50 border border-violet-200 px-2 py-0.5 rounded-md shrink-0 ml-2">
            {String(item.subCategory || 'CREATIVE').toUpperCase()}
          </span>
        </div>
...
export default function CreatorsFeed({
  ...
  const categoryConfig = getCategoryById('creators') || { subCategories: [] };
  ...
  const getMessageTemplate = (item) => {
    return `Namaste ${item.name || ''}, I found your creative profile on TownHub Creators. I have a project/video requirement in ${selectedCity}. Can we discuss?`;
  };
      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500">
            {filteredListings.length} verified creators, editors & studios in {selectedCity}
          </p>
        </div>
        <button
          type="button"
