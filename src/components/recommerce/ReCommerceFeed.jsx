...
export default function ReCommerceFeed({
  ...
  const categoryConfig = getCategoryById('recommerce') || { subCategories: [] };
  ...
  const getMessageTemplate = (item) => {
    return `Namaste ${item.sellerName || ''}, I saw your second-hand listing "${item.title || item.name}" on TownHub in ${selectedCity}. Is it still available to inspect?`;
  };
      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500">
            {filteredListings.length} live citizen listings in {selectedCity}
          </p>
        </div>
        <button
          type="button"
