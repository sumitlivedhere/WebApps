...
export default function MallsFeed({
  ...
  const filtered = stores
    .filter((item) => {
      if (selectedCategoryId && selectedCategoryId !== 'all') return item.type === selectedCategoryId || item.category === selectedCategoryId;
      return true;
    })
    .filter((item) => {
      if (!searchQuery) return true;
      const q = searchQuery.toLowerCase();
      return item.name?.toLowerCase().includes(q) || item.mallName?.toLowerCase().includes(q) || item.offer?.toLowerCase().includes(q);
    });
      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
        <div>
          <h2 className="text-sm font-black text-slate-900 capitalize leading-tight">
            {categoryTitle || 'Malls, Boutiques & Showrooms'}
          </h2>
          <p className="text-[10px] text-slate-500">Live store offers in {selectedCity}</p>
        </div>
      ...
      {filtered.map((item) => (
        <article key={item.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-200 hover:shadow-md transition duration-200 space-y-3 p-3.5 relative">
          <div className="relative h-52 w-full bg-slate-100 rounded-2xl overflow-hidden shadow-inner">
            <img src={item.image || 'https://images.unsplash.com/photo-1567401893414-76b7b1e5a7a5?w=700'} alt={item.name} className="w-full h-full object-cover" />
            ...
            <ListingDiscussionThread
              ...
              interestCount={item.interestCount || 9}
          ...
          <div>
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs font-bold text-indigo-700 mt-0.5">👗 {item.categoryLabel || item.collection}</p>
              </div>
              <span className="bg-rose-100 text-rose-800 text-[10px] font-black px-2 py-0.5 rounded-md ml-2 shrink-0">
                {item.discount || 'Flat 20% Off'}
              </span>
            </div>
          ...
          <ActionButtons
            ...
            message={`Namaste, I want to check latest stock & discount at *${item.name}*.`}
          />
        </article>
      ))}
    </main>
