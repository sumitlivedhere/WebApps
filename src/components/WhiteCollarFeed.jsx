      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
        <div>
          <h2 className="text-sm font-black text-slate-900 capitalize">
            {selectedSubCategory !== 'all' ? selectedSubCategory.replace('-', ' ') : 'All Professionals'}
          </h2>
          <p className="text-[10px] text-slate-500">Doctors, CA, Lawyers & Consultants in {selectedCity}</p>
        </div>
        <button
          type="button"
          onClick={onBack}
      ...
      {filteredListings.length === 0 ? (
        <div className="bg-white/80 rounded-2xl p-8 text-center border border-slate-200">
          ...
          <p className="text-slate-600 font-bold text-xs mt-2">
            No professionals listed in this category for {selectedCity}.
          </p>
        </div>
      ) : (
        filteredListings.map((item) => (
          ...
          >
            ...
            <ActionButtons
              ...
              message={`Namaste ${item.name}, I am reaching out regarding a consultation.`}
            />
          </article>
        ))
      )}
