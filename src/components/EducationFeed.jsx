...
export default function EducationFeed({
  ...
  const filtered = listings
    .filter((item) => {
      if (selectedExamId && selectedExamId !== 'all') return item.examCategory === selectedExamId || item.category === selectedExamId;
      return true;
    })
    .filter((item) => {
      if (!searchQuery) return true;
      const q = searchQuery.toLowerCase();
      return item.name?.toLowerCase().includes(q) || item.subjects?.join(' ').toLowerCase().includes(q);
    });
      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
        <div>
          <h2 className="text-sm font-black text-slate-900 capitalize leading-tight">
            {examTitle || 'Coaching & Home Tuitions'}
          </h2>
      ...
      {filtered.map((item) => (
        <article key={item.id} className="bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-200 hover:shadow-md transition duration-200 space-y-3 p-3.5 relative">
          <div className="relative h-48 w-full bg-slate-100 rounded-2xl overflow-hidden shadow-inner">
            <img src={item.image || 'https://images.unsplash.com/photo-1524178232363-1fb2b075b655?w=700'} alt={item.name} className="w-full h-full object-cover" />
            ...
            <ListingDiscussionThread
              ...
              interestCount={item.interestCount || 8}
          ...
          <div>
            <div className="flex items-start justify-between">
              <div>
                ...
                <p className="text-xs font-bold text-indigo-700 mt-0.5">🎓 {item.subjects?.join(', ') || 'All Subjects'}</p>
              </div>
              <span className="bg-amber-100 text-amber-900 text-[10px] font-bold px-2 py-0.5 rounded-md ml-2 shrink-0">
                {item.format || 'Offline + Online'}
              </span>
            </div>
            {item.achievements && <p className="text-[11px] text-slate-600 mt-1 line-clamp-2 leading-relaxed">🏆 {item.achievements}</p>}
            <div className="flex items-center justify-between text-[10px] text-slate-500 font-semibold mt-2 pt-2 border-t border-slate-100">
              ...
              <span className="text-emerald-700 font-bold">{item.distance || '0.6 km away'}</span>
            </div>
          </div>
          ...
          <ActionButtons
            ...
            message={`Namaste Sir/Mam, I want to inquire about batches/tuition for "${item.name}".`}
          />
        </article>
      ))}
    </main>
