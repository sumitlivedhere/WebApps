...
function EducationCardItem({ item, selectedCity, onSelect, getMessageTemplate }) {
  ...
  const handleStarClick = (e) => {
    ...
    hyperlocalStore.incrementInterest(
      ...
      item.teacherName || item.instituteName || item.sellerName || 'Education Institute / Tutor'
    );
  };
    ...
    >
      ...
      <div className="relative h-48 w-full bg-slate-100 rounded-2xl overflow-hidden shadow-inner select-none">
        ...
        <span className="absolute bottom-2.5 left-2.5 text-xs font-black px-2.5 py-1 rounded-xl text-white bg-slate-950/85 backdrop-blur-md border border-white/10">
          {item.fee || item.price || item.rates || 'Fee on Inquiry'}
        </span>
      ...
      <div className="pt-0.5 space-y-1.5">
        <div className="flex items-start justify-between">
          <div className="min-w-0">
            ...
            {(item.teacherName || item.instituteName || item.sellerName) && (
          ...
          <span className="text-[10px] font-bold text-indigo-800 bg-indigo-50 border border-indigo-200 px-2 py-0.5 rounded-md shrink-0 ml-2">
            {String(item.subCategory || 'EDUCATION').toUpperCase()}
          </span>
        </div>
...
export default function EducationFeed({
  ...
  const categoryConfig = getCategoryById('education') || { subCategories: [] };
  ...
  const filteredListings = useMemo(() => {
    ...
    storeListings.forEach((item) => {
      ...
      // 2. Subcategory Match
      const itemSub = (item.subCategory || item.category || item.tuitionType || '').toLowerCase().trim();
  ...
  const getMessageTemplate = (item) => {
    const title = item.title || item.name || 'Course/Tuition';
    return `Namaste ${item.teacherName || item.instituteName || item.sellerName || ''}, I found your education listing "${title}" on TownHub in ${selectedCity}. I want to inquire about batch timings, syllabus & admission details.`;
  };
      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500">
            {filteredListings.length} verified educators, institutes & coaching centers in {selectedCity}
          </p>
        </div>
        <button
          type="button"
      ...
      {filteredListings.length === 0 ? (
        <div className="bg-white/80 rounded-2xl p-8 text-center border border-slate-200">
          ...
          <p className="text-slate-600 font-bold text-xs mt-2">
            No educational listings found under {targetSub !== 'all' ? targetSub : 'this category'} in {selectedCity}.
          </p>
        </div>
      ) : (
