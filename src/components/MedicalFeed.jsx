...
function MedicalCardItem({ item, selectedCity, onSelect, getMessageTemplate }) {
  ...
  const handleStarClick = (e) => {
    ...
    hyperlocalStore.incrementInterest(
      ...
      item.doctorName || item.sellerName || 'Doctor Clinic'
    );
  };
  ...
  return (
    <article
      ...
      className={`bg-white rounded-2xl overflow-hidden shadow-xs border transition p-3.5 space-y-3 relative cursor-pointer hover:shadow-md active:scale-99 ${
        item.isNew ? 'border-red-400 ring-2 ring-red-400/20' : 'border-slate-200'
      }`}
    >
      {/* Photo Banner */}
      <div className="relative h-48 w-full bg-slate-100 rounded-2xl overflow-hidden shadow-inner select-none">
        ...
        <span className="absolute bottom-2.5 left-2.5 text-xs font-black px-2.5 py-1 rounded-xl text-white bg-slate-950/85 backdrop-blur-md border border-white/10">
          {item.fee || item.price || item.rates || 'OPD Fee on Request'}
        </span>
      ...
      <div className="pt-0.5 space-y-1.5">
        <div className="flex items-start justify-between">
          <div className="min-w-0">
            ...
            {item.qualifications && (
              <p className="text-[10px] text-red-700 font-bold mt-0.5 truncate">
                🎓 {item.qualifications}
              </p>
            )}
            {item.regNumber && (
...
export default function MedicalFeed({
  ...
  const categoryConfig = getCategoryById('medical') || { subCategories: [] };
  ...
  const getMessageTemplate = (item) => {
    const docName = item.doctorName || item.sellerName || item.name || '';
    return `Namaste ${docName}, I found your clinic/doctor profile on TownHub Medical in ${selectedCity}. I want to inquire about OPD consultation timings & appointment availability.`;
  };
      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500">
            {filteredListings.length} verified clinics, doctors & hospitals in {selectedCity}
          </p>
        </div>
        <button
          type="button"
