...
function FitnessCardItem({ item, selectedCity, onSelect, getMessageTemplate }) {
  ...
  const handleStarClick = (e) => {
    ...
    hyperlocalStore.incrementInterest(
      ...
      item.trainerName || item.sellerName || item.name || 'Fitness Trainer / Gym'
    );
  };
  ...
  const gallery =
    item.images && item.images.length > 0
      ...
      : item.image
      ? [item.image]
      : ['https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=700'];
    ...
    >
      ...
      <div className="relative h-48 w-full bg-slate-100 rounded-2xl overflow-hidden shadow-inner select-none">
        <img
          ...
          onError={(e) => {
            e.target.src = 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=700';
          }}
        />

        {/* Price / Membership Fee Tag */}
        <span className="absolute bottom-2.5 left-2.5 text-xs font-black px-2.5 py-1 rounded-xl text-white bg-slate-950/85 backdrop-blur-md border border-white/10">
          {item.price || item.fee || item.rates || 'Fees on Request'}
        </span>
      ...
      <div className="pt-0.5 space-y-1.5">
        <div className="flex items-start justify-between">
          <div className="min-w-0">
            ...
            {(item.trainerName || item.sellerName) && (
              <p className="text-[10px] text-emerald-700 font-bold mt-0.5 truncate">
                🏋️ {item.trainerName || item.sellerName}
              </p>
            )}
          </div>
          <span className="text-[10px] font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-md shrink-0 ml-2">
            {String(item.subCategory || 'FITNESS').toUpperCase()}
          </span>
        </div>
...
export default function FitnessFeed({
  ...
  const storeListings = useStoreSlice('fitnessListings') || [];
  const targetSub = (selectedSubCategory || selectedCategory || 'all').toLowerCase().trim();
  const categoryConfig = getCategoryById('fitness') || { subCategories: [] };
  ...
  const getSubCategoryTitle = () => {
    if (targetSub === 'all') return 'All Fitness, Gyms & Sports';
  ...
  const getMessageTemplate = (item) => {
    return `Namaste ${item.trainerName || item.sellerName || ''}, I found "${item.name || item.title}" on TownHub Fitness. I want to inquire about membership plans, timings & trials in ${selectedCity}.`;
  };
      ...
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500">
            {filteredListings.length} verified fitness centers & trainers in {selectedCity}
          </p>
        </div>
        <button
          type="button"
