    ...
    <main className="p-3.5 space-y-3.5 relative z-10 animate-fade-in text-slate-800 pb-16">
      <div className="bg-white/85 backdrop-blur-md p-3.5 rounded-2xl border border-slate-200 shadow-sm flex items-center justify-between">
        <div>
          ...
          <p className="text-[10px] text-slate-500">Live verified fitness centers & trainers in {selectedCity}</p>
        </div>
        <button
          type="button"
          onClick={onBack}
        ...
        filteredListings.map((item) => (
          ...
          >
            <div className="relative h-44 w-full bg-slate-100 rounded-2xl overflow-hidden shadow-inner">
              <img
                src={item.image || 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=700'}
                alt={item.name}
                loading="lazy"
              ...
              <ListingDiscussionThread
                ...
                sellerName={item.trainerName || item.name}
            ...
            <div className="pt-0.5 space-y-1">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {item.trainerName && (
                    <p className="text-[10px] text-emerald-700 font-bold mt-0.5">👤 {item.trainerName}</p>
                  )}
                </div>
                <span className="text-[10px] font-bold text-emerald-800 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-md shrink-0 ml-2">
                  {(item.subCategory || 'FITNESS').toUpperCase()}
                </span>
              </div>
              ...
              <div className="flex items-center justify-between text-[10px] text-slate-500 font-semibold pt-2 border-t border-slate-100">
                ...
                <span className="text-emerald-700 font-bold">{item.badge || item.timing || '🟢 Open Today'}</span>
              </div>
            </div>
            ...
            <ActionButtons
              ...
              message={`Namaste, I found "${item.name}" on TownHub Fitness. I want to inquire about membership fees & trial in ${selectedCity}.`}
            />
          </article>
        ))
      )}
