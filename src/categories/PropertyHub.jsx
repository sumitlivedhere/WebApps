...
export default function PropertyHub({
  ...
  const handleSelect = (subId) => {
    ...
    } else if (typeof onSelectPropertyType === 'function') {
      onSelectPropertyType(subId);
    }
  };
  ...
  return (
    <div className="p-3.5 space-y-4 animate-fade-in text-slate-800">
      {/* Header */}
      <div className="bg-gradient-to-r from-indigo-900 via-indigo-800 to-slate-900 p-4 rounded-3xl text-white shadow-md flex items-center justify-between border border-indigo-700/50">
        <div>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        ...
        >
          ...
          <div>
            ...
            <div className="text-[9px] text-slate-400 font-normal">All property listings</div>
          </div>
        </button>
        ...
        {categoryConfig.subCategories.map((sub) => (
          <button
            ...
            className="p-3.5 bg-white text-slate-900 rounded-2xl text-left font-bold shadow-sm hover:scale-[1.02] active:scale-95 transition cursor-pointer flex flex-col justify-between h-24 border border-slate-200 hover:border-indigo-400"
          >
            <span className="text-xl">📍</span>
            <div>
              <div className="text-xs font-black leading-tight">{sub.name.split('(')[0]}</div>
              <div className="text-[9px] text-slate-500 font-semibold">
                {sub.name.match(/\((.*?)\)/)?.[1] || 'प्रॉपर्टी'}
              </div>
            </div>
          </button>
        ))}
