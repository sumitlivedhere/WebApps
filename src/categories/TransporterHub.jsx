...
export default function TransporterHub({
  ...
  const handleSelect = (subId) => {
    ...
    } else if (typeof onSelectIndividualVehicle === 'function') {
      onSelectIndividualVehicle(subId);
    } else if (typeof onSelectFirms === 'function') {
      onSelectFirms();
    }
  };
  ...
  return (
    <div className="p-3.5 space-y-4 animate-fade-in text-slate-800">
      {/* Header */}
      <div className="bg-gradient-to-r from-amber-600 via-orange-600 to-amber-700 p-4 rounded-3xl text-white shadow-md flex items-center justify-between border border-amber-400/30">
        <div>
          <span className="text-2xl block">🚚</span>
          <h2 className="text-base font-black leading-tight mt-1 text-yellow-200">
            Transporters & Loading (ट्रांसपोर्ट)
          </h2>
          <p className="text-[11px] font-semibold text-amber-100">
            Pickup, Loading Auto & Heavy Transport in {selectedCity}
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="grid grid-cols-2 gap-2.5">
        ...
        >
          ...
          <div>
            ...
            <div className="text-[9px] text-slate-400 font-normal">All commercial transport</div>
          </div>
        </button>
