...
export default function TransporterHub({ selectedCity = 'Alwar', onSelectSubCategory, onBack }) {
  const categoryConfig = getCategoryById('transporters');

  return (
    <div className="p-3.5 space-y-4 animate-fade-in text-slate-800">
      <div className="bg-gradient-to-r from-amber-600 via-orange-600 to-amber-700 p-4 rounded-3xl text-white shadow-md flex items-center justify-between">
        <div>
          ...
          <p className="text-[11px] font-semibold text-amber-100">
            Pickup, Loading Auto & Logistics in {selectedCity}
          </p>
        </div>
        <button
          type="button"
        ...
        {categoryConfig.subCategories.map((sub) => (
          ...
          >
            ...
            <div>
              ...
              <div className="text-[9px] text-slate-500 font-semibold">{sub.name.match(/\((.*?)\)/)?.[1] || 'गाड़ी'}</div>
            </div>
          </button>
        ))}
      </div>
