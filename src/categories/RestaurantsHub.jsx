...
export default function RestaurantsHub({ selectedCity = 'Alwar', onSelectSubCategory, onBack }) {
  const categoryConfig = getCategoryById('restaurants');

  return (
    <div className="p-3.5 space-y-4 animate-fade-in text-slate-800">
      <div className="bg-gradient-to-r from-rose-600 via-red-600 to-amber-600 p-4 rounded-3xl text-white shadow-md flex items-center justify-between">
        <div>
        ...
        {categoryConfig.subCategories.map((sub) => (
          ...
          >
            ...
            <div>
              ...
              <div className="text-[9px] text-slate-500 font-semibold">{sub.name.match(/\((.*?)\)/)?.[1] || 'भोजन'}</div>
            </div>
          </button>
        ))}
      </div>
