...
const EMERGENCY_SERVICES = [
  { subId: 'plumbers-water-motor', label: '💧 Water Pipe Leak / Motor Jam', icon: '🚰', desc: 'Plumber in 20-30 mins' },
  { subId: 'electricians-inverter', label: '⚡ MCB Trip / Short Circuit', icon: '⚡', desc: 'Electrician on call' },
  { subId: 'locksmith-key-maker', label: '🔑 Locked Out / Broken Key', icon: '🔑', desc: 'Doorstep key maker' },
  { subId: 'mechanic-puncture', label: '🛞 Flat Tyre / Roadside Breakdown', icon: '🛞', desc: 'Mobile puncture & mechanic' },
];
  ...
  return (
    <div className="p-3.5 space-y-4 animate-fade-in text-slate-100 pb-16">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-950 via-slate-900 to-cyan-950 p-4 border border-blue-500/40 shadow-2xl">
        <div className="absolute -top-12 -right-12 w-44 h-44 bg-blue-500 rounded-full blur-3xl opacity-20 pointer-events-none"></div>
          ...
          {EMERGENCY_SERVICES.map((sos) => (
            ...
            >
              <div className="space-y-0.5">
                <div className="text-[11px] font-black text-slate-100 leading-tight flex items-center space-x-1">
                  <span>{sos.label}</span>
                </div>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <div className="flex items-center space-x-1.5">
                    ...
                    {sub.tag && (
                      <span className="text-[8px] font-black px-1.5 py-0.2 rounded-md bg-blue-500/20 text-cyan-300 border border-blue-400/30">
                        {sub.tag}
                      </span>
                    )}
                  </div>
                  <div className="text-[9px] text-slate-400 font-semibold mt-0.5 truncate">
                    {sub.name.match(/\((.*?)\)/)?.[1] || 'कारीगर'}
                  </div>
                </div>
              </div>
      ...
      {/* 🌟 4. WORKER REGISTRATION BANNER */}
      <div className="p-3.5 bg-gradient-to-r from-slate-900 via-blue-950/60 to-slate-900 border border-blue-600/40 rounded-2xl flex items-center justify-between shadow-sm">
        <div className="flex items-center space-x-2.5">
          <span className="text-2xl">👷</span>
          <div>
            <div className="text-xs font-black text-cyan-300">Are you a Mistri, Electrician or Plumber?</div>
            <div className="text-[10px] text-slate-300">List your visiting fee, skills & phone number for direct customer calls in {selectedCity}. Zero commission.</div>
          </div>
        </div>
